...
void dae::IngredientComponent::Update(float)
{
	...
	const auto posIngredientBase  = locationIngredient->GetTransform().GetPosition();
	...
	if (!inMotion)
	{
		for (const auto& player : levelManager.GetPlayers())
		{
			...
			const auto posPlayer      = locationPlayer->GetTransform().GetPosition();
			...
			for (int i = 0; i < 3; i++)
			{
				...
				if (posIngredient.y == (int(posPlayer.y)) &&
					playerCenterX > posIngredient.x && playerCenterX < (posIngredient.x + GridComponent::GridCellSize.
						x))
				{
					marked[i] = true;
